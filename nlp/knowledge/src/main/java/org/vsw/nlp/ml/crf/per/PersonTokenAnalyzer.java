...
abstract public class PersonTokenAnalyzer implements TokenAnalyzer {
	private TokenFeaturesGenerator featuresGenerator ;
	private CRF crf ;
	private WTagInstanceFactory factory ;
	
	public PersonTokenAnalyzer(String crfModelFile, PersonSetConfig config, NLPResource resource) throws Exception {
	  featuresGenerator = PersonTrainer.newTokenFeaturesGenerator(config) ;
    ObjectInputStream s = new ObjectInputStream(IOUtil.loadRes(crfModelFile));
