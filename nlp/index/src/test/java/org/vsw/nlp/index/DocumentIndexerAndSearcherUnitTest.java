...
public class DocumentIndexerAndSearcherUnitTest {
  ...
  public void test() throws Exception {
    FSDocumentIndexer indexer = new FSDocumentIndexer("target", "index", "shard0") ;
    int NUM_DOC = 10 ;
    ...
    FSDocumentSearcher searcher = new FSDocumentSearcher("target", "index", "shard0") ;
  ...
  private void expect(FSDocumentSearcher searcher, String query, int expect) throws Exception{
    TopDocsCollector collector = searcher.query(query) ;
    Assert.assertEquals(expect, collector.getTotalHits()) ;
  }
  
  private void expect(FSDocumentSearcher searcher, Query query, int expect) throws Exception{
